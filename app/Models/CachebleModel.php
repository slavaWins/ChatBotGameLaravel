...
class CachebleModel extends Model
{
    ...
    public const CACHE_KEY_BASE = null;
    ...
    {
        if(self::CACHE_DISABLED)parent::find($id);
        ...
        if ($model) $model->CacheSave();
        return $model;
    }

    public static function CacheKeyById($id)
    {
        if (get_called_class()::CACHE_KEY_BASE) {
            return get_called_class()::CACHE_KEY_BASE . $id;
        }
    ...
    {
        if(self::CACHE_DISABLED)parent::save($options);
        //  unset($data['exist']);
        $this->CacheClear();
